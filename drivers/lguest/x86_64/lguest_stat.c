...
void lguest_stat_return_to_host(struct lguest_vcpu *vcpu)
{
	struct lguest *lg = vcpu->guest;
	struct lguest_vcpu_data *data = vcpu->vcpu_data;
	unsigned long cnt;

	spin_lock(&lguest_stat_lock);
	if (data->regs.trapnum == LGUEST_TRAP_ENTRY) {
		cnt = data->regs.rax;
		if (unlikely(cnt > LGUEST_MAX_HCALLS))
			cnt = LGUEST_MAX_HCALLS;
		lg->stat_hcalls[cnt]++;
		lguest_total_hcalls[cnt]++;
		vcpu->stat_cause = STAT_HCALL(cnt);
	}

	cnt = data->regs.trapnum;

	if (cnt != LGUEST_TRAP_ENTRY)
		vcpu->stat_cause = STAT_TRAP(cnt);
...
void lguest_stat_start_pagefault(struct lguest_vcpu *vcpu)
{
	vcpu->stat_pf = sched_clock();
}

void lguest_stat_end_pagefault(struct lguest_vcpu *vcpu)
{
	u64 end = sched_clock();
...
void lguest_stat_start_time(struct lguest_vcpu *vcpu)
{
	vcpu->stat_time = sched_clock();
	if (!vcpu->stat_time) /* off by one is ok ;-)  */
		vcpu->stat_time = 1;
}

void lguest_stat_end_time(struct lguest_vcpu *vcpu)
{
	u64 end;
	...
	if (vcpu->stat_cause & STAT_HCALL_FL) {
		lguest_total_hcall_times[vcpu->stat_cause & STAT_MASK] += end;
		lguest_total_trap_times[LGUEST_TRAP_ENTRY] += end;
	} else if (vcpu->stat_cause & STAT_TRAP_FL)
		lguest_total_trap_times[vcpu->stat_cause & STAT_MASK] += end;

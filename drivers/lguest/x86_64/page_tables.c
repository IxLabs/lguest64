 ...
 *  or any other guest.
 *
 *  The VCPU Data is read only by both the guest and the host, and that
 ...
 *
 *  The VCPU points to a unique page descriptor (pgd) that represents some
...
static struct lguest_pg * lguest_alloc_pg(struct lguest_vcpu *vcpu,
					  u64 *haddr, u64 *ptr,
					  unsigned long val)
{
...
#if EXPENSIVE_TESTS
		LGUEST_WARN_ON_DUMP(vcpu, find_hpg(lg, __va(*ptr & PTE_MASK)), goto out);
#endif
		/* Flag it as mapped */
		*ptr |= LGUEST_PAGE_PRESENT;
	}

	LGUEST_WARN_ON_DUMP(vcpu, (u64)haddr & (PAGE_SIZE-1), goto out);
	...
	LGUEST_WARN_ON_DUMP(vcpu, find_hpg(lg, pg->haddr), return NULL);
#endif
...
static u64* lguest_map_pud(struct lguest_vcpu *vcpu,
			   u64 *pgd,
			   unsigned long val)
{
	struct lguest_pg *pg;
	...
	if (*pgd & _PAGE_PRESENT) {
		...
		LGUEST_WARN_ON_DUMP(vcpu, !(*pgd & LGUEST_PAGE_HV), );
		haddr = __va(*pgd & PTE_MASK);
	}

	pg = lguest_alloc_pg(vcpu, haddr, pgd, val);
	if (!pg)
		return NULL;
...
static u64* lguest_map_pmd(struct lguest_vcpu *vcpu,
			   u64 *pud,
			   unsigned long val)
{
	struct lguest_pg *pg;
	...
	if (*pud & _PAGE_PRESENT) {
		/* Better be a HV page */
		LGUEST_WARN_ON_DUMP(vcpu, !(*pud & LGUEST_PAGE_HV), );
		/* Aha! this is paged in, but not mapped yet. */
		haddr = __va(*pud & PTE_MASK);
	}

	pg = lguest_alloc_pg(vcpu, haddr, pud, val);
	if (!pg)
		return NULL;
...
static u64* lguest_map_pte(struct lguest_vcpu *vcpu,
			   u64 *pmd,
			   unsigned long val)
{
	struct lguest_pg *pg;
	...
	if (unlikely(*pmd & _PAGE_PRESENT)) {
		printk("PMD present not expected pmd=%p *pmd=%llx\n",
		       pmd, *pmd);
		pg = find_hpg(vcpu->guest, __va(*pmd & PTE_MASK));
		if (pg) {
			printk("*pmd pg found?? %p haddr=%p gaddr=%llx pgd=%p\n",
			       pg, pg->haddr, pg->gaddr, pg->pgd);
		}
		kill_guest_dump(vcpu, "bad pmd?");
		return NULL;
	}

	pg = lguest_alloc_pg(vcpu, NULL, pmd, val);
	if (!pg)
		return NULL;
...
static struct lguest_pg *dup_pg(struct lguest_vcpu *vcpu,
				struct lguest_pg *pg,
				u64 *ptr)
{
	...
	LGUEST_WARN_ON_DUMP(vcpu, pg->count > 1, return NULL);

	dpg = lguest_alloc_pg(vcpu, __va(*ptr & PTE_MASK), ptr, pg->gaddr);
	if (!dpg)
		return NULL;
...
static u64 *map_pg_to_haddr(struct lguest_vcpu *vcpu,
			    struct lguest_pg *pg,
			    u64 *ptr,
			    u64 val)
{
	if (!(*ptr & LGUEST_PAGE_PRESENT)) {
		...
		if (*ptr & _PAGE_PRESENT) {
			LGUEST_WARN_ON_DUMP(vcpu, !(*ptr & LGUEST_PAGE_HV), return NULL);
			pg = dup_pg(vcpu, pg, ptr);
		} else {
		...
		list_for_each_entry(lpg, &pg->copies, copies)
			LGUEST_WARN_ON_DUMP(vcpu, pg->gaddr != lpg->gaddr, return NULL);
#endif
		...
		LGUEST_WARN_ON_DUMP(vcpu, 1, return NULL);
	}
...
#if 0
static int lguest_map_pg(struct lguest_vcpu *vcpu,
			 u64 *ptr,
			 u64 gaddr,
			 u64 val,
			 enum lguest_pg_type type)
{
	...
	lgwrite_u64(vcpu, gaddr, val);

	pg = find_pg(lg, gud);
	if (pg) {
		LGUEST_WARN_ON_DUMP(vcpu, pg->type != type, return -1);
		p = map_pg_to_haddr(vcpu, pg, ptr, val);
	} else {
		switch (type) {
		case LGUEST_PG_PUD:
			p = lguest_map_pud(vcpu, ptr, val);
			break;
		case LGUEST_PG_PMD:
			p = lguest_map_pmd(vcpu, ptr, val);
			break;
		case LGUEST_PG_PTE:
			p = lguest_map_pte(vcpu, ptr, val);
			break;
		default:
			kill_guest_dump(vcpu, "Unknow type??");
			return -1;
		}
		LGUEST_WARN_ON_DUMP(vcpu, !p, return -1);
	}

	vcpu->vcpu_data->page_set = 1;
...
static int page_in(struct lguest_vcpu *vcpu, u64 vaddr, int flags)
{
	struct lguest *lg = vcpu->guest;
	u64 pgprot;
	u64 val;
	...
	lgdebug_lprint(LGD_PG_FL, "\nvaddr=%llx pgd=%p rip=%llx\n",
		       vaddr, vcpu->pgd, vcpu->vcpu_data->regs.rip);
	/* If vaddr is a HV page, then kill the guest */
	if (contains_hv_page(vaddr, 1)) {
		printk("hv start: %lx  end: %lx rip=%llx\n",
		       lguest_hv_start, lguest_hv_start + lguest_hv_size, vcpu->vcpu_data->regs.rip);
		printk("vaddr=%llx  flags=%d (%s)\n",
		       vaddr, flags, flags & _PAGE_DIRTY ? "write":"read");
		if (0) /* don't kill it, just send a fault */
			kill_guest_dump(vcpu, "Guest mapping HV page %llx\n",vaddr);
		return 0;
	}

	LGUEST_WARN_ON_DUMP(vcpu, !is_canonical(vaddr), return 0);
	...
	LGUEST_WARN_ON_DUMP(vcpu, (*hpgd & LGUEST_PAGE_PRESENT) && !pg, goto out);

	if (pg) {
		LGUEST_WARN_ON_DUMP(vcpu, pg->type != LGUEST_PG_PUD, goto out);
		hpud = map_pg_to_haddr(vcpu, pg, hpgd, val);
	} else
		hpud = lguest_map_pud(vcpu, hpgd, val);
	lgdebug_lprint(LGD_PG_FL, "hpud=%p *hpgd=%llx\n", hpud, *hpgd);

	LGUEST_WARN_ON_DUMP(vcpu, !hpud, goto out);
	...
	LGUEST_WARN_ON_DUMP(vcpu, (*hpud & LGUEST_PAGE_PRESENT) && !pg, goto out);

	if (pg) {
		LGUEST_WARN_ON_DUMP(vcpu, pg->type != LGUEST_PG_PMD, goto out);
		hpmd = map_pg_to_haddr(vcpu, pg, hpud, val);
	} else
		hpmd = lguest_map_pmd(vcpu, hpud, val);
	...
	if (val & _PAGE_PSE) {
		...
		LGUEST_WARN_ON_DUMP(vcpu, *hpmd & LGUEST_PAGE_HV, goto out);
		/*
	...
	if (((paddr & PTE_MASK)>>PAGE_SHIFT) > lg->pfn_limit) {
		kill_guest_dump(vcpu, "accessing page %lld over limit %lld\n",
	...
	if (pg) {
		LGUEST_WARN_ON_DUMP(vcpu, pg->type != LGUEST_PG_PTE, goto out);
		...
		hpte = lguest_map_pte(vcpu, hpmd, val);
...
out:
	 ...
	 */
	if (ret || (vcpu->vcpu_data->regs.cs & 3) == 3)
		/* All is ok, we handled it or it's user space */
		vcpu->vcpu_data->last_pgd = NULL;
	else {
		if ((vcpu->vcpu_data->last_pgd == vcpu->pgd) &&
		    (vcpu->vcpu_data->last_rip == vcpu->vcpu_data->regs.rip) &&
		    (vcpu->vcpu_data->last_vaddr == vaddr))
			/* FIXME: send double fault to guest instead */
			kill_guest_dump(vcpu, "double fault at %llx RIP: %llx",
					vaddr, vcpu->vcpu_data->last_rip);
		else {
			vcpu->vcpu_data->last_pgd = vcpu->pgd;
			vcpu->vcpu_data->last_rip = vcpu->vcpu_data->regs.rip;
			vcpu->vcpu_data->last_vaddr = vaddr;
		}
	}
...
{
	int ret;
	lguest_stat_start_pagefault(vcpu);
	ret = page_in(vcpu, vaddr, (write ? _PAGE_DIRTY : 0)|_PAGE_ACCESSED);
	lguest_stat_end_pagefault(vcpu);
	return ret;
}
		   ...
		   unsigned long val)
{
	...
	lguest_data_set_bit(PGSET, vcpu->vcpu_data);
		   ...
		   unsigned long val)
{
	...
	if (unlikely(pg->type != LGUEST_PG_PUD)) {
		...
		for (i=0; i < lg->nr_pgds; i++) {
			if (pg->pgd == &lg->pgds[i]) {
				...
				lguest_print_guest_page_tables(vcpu, pg->pgd->gcr3);
				break;
			}
		}
		if (i==lg->nr_pgds)
	...
	list_for_each_entry(lpg, &pg->copies, copies) {
		LGUEST_WARN_ON_DUMP(vcpu, !cmp_gaddr(pg, lpg), goto out);
...
static void __guest_set_pte(struct lguest_vcpu *vcpu,
			    unsigned long gaddr,
			    unsigned long val)
{
	struct lguest *lg = vcpu->guest;
	...
	if (unlikely(*hpte & LGUEST_PAGE_HV)) {
		kill_guest_dump(vcpu, "guest freeing HV page??");
		goto out;
	}
	...
	if (paddr == (unsigned long)-1UL) {
		kill_guest_dump(vcpu, "page %lx not mapped", val);
		goto out;
	}
...
void guest_set_pte(struct lguest_vcpu *vcpu,
		   ...
		   unsigned long val)
{
	__guest_set_pte(vcpu, gaddr, val);
}
...
static void release_pgd(struct lguest *lg, struct lguest_pgd *pgd)
{
	...
	lgdebug_lprint(LGD_PG_FL, "release_pgd %p vcpu=%p hcr3=%p gcr3=%llx\n",
		       pgd, pgd->vcpu, pgd->hcr3, pgd->gcr3);
	pud = pgd->hcr3;
...
void guest_release_pgd(struct lguest_vcpu *vcpu, u64 gaddr)
{
	struct lguest *lg = vcpu->guest;
	struct lguest_pgd *pgd;
	u64 gcr3 = gaddr & PTE_MASK;
...
{
	struct lguest *lg = vcpu->guest;
	__guest_pagetable_clear_all(lg);
}

static void assign_pgd_vcpu(struct lguest_vcpu *vcpu,
			    struct lguest_pgd *pgd)
{
	struct lguest *lg = vcpu->guest;
	u64 *pud;
	...
	set_pt(pmd[idx], __pa(vcpu->hv_pte) | LGUEST_HV_PG_PROT);
	...
	if (likely(vcpu->pgd)) {
		vcpu->pgd->flags &= ~LGUEST_PGD_BUSY_FL;
		list_add(&vcpu->pgd->lru, &lg->pgd_lru);
	}
...
static struct lguest_pgd *allocate_new_pgd(struct lguest_vcpu *vcpu, u64 gcr3)
{
	struct lguest *lg = vcpu->guest;
	struct lguest_pgd *pgd;
	int idx;
	...
	if (lg->nr_pgds < LGUEST_PGD_SIZE) {
		...
		LGUEST_WARN_ON_DUMP(vcpu, pgd->hcr3, return NULL);
		...
		LGUEST_WARN_ON_DUMP(vcpu, pgd->flags & LGUEST_PGD_BUSY_FL, return NULL);
...
void guest_new_pagetable(struct lguest_vcpu *vcpu, u64 cr3)
{
	...
	if (pgd->vcpu != vcpu) {
		...
		if (lpgd == pgd) {
			...
			/* Make a new copy */
			lpgd = allocate_new_pgd(vcpu, cr3);
...
int lguest_init_vcpu_pagetable(struct lguest_vcpu *vcpu)
{
	...
	guest_new_pagetable(vcpu, lg->cr3);
 ...
 */
u64 lguest_find_guest_paddr(struct lguest_vcpu *vcpu, u64 vaddr)
{
	struct lguest *lg = vcpu->guest;
	u64 *gcr3;
	u64 *gpgd;
...
static void lguest_pte_map_vcpu_data(struct lguest_vcpu *vcpu, u64 *ptr, int prot)
{
	unsigned long page;
	unsigned long vaddr;
	unsigned long daddr;
	...
	vaddr = (unsigned long)vcpu;
	daddr = lguest_vcpu_addr;
	for (i=0; i < lguest_vcpu_pages; i++) {
...
static void lguest_pte_map_vcpu_guest_data(struct lguest_vcpu *vcpu, u64 *ptr, int prot)
{
	unsigned long page;
	unsigned long vaddr;
	unsigned long daddr;
	...
	vaddr = (unsigned long)vcpu->vcpu_data;
	daddr = lguest_vcpu_data_addr;
	for (i=0; i < lguest_vcpu_data_pages; i++) {
...
int lguest_map_guest_vcpu(struct lguest_vcpu *vcpu)
{
	...
	lguest_pte_map_vcpu_data(vcpu, ptr, LGUEST_HV_DATA_PROT);

	/* Last, make a guest data that the guest can write in */
	lguest_pte_map_vcpu_guest_data(vcpu, ptr, LGUEST_HV_VCPU_DATA_PROT);
...
int lguest_update_page_tables(struct lguest_vcpu *vcpu)
{
	...
	vcpu->guest_cr3 = __pa(vcpu->pgd->hcr3);
	vcpu->gcr3 = vcpu->pgd->gcr3;
	...
	/*
	 * If this is already mapped to the current VCPU
	 * than we are done.
	 */
	idx = pte_index(lguest_vcpu_addr);
	if (pte[idx] == vcpu->hv_pte[idx])
		return 0;
	 ...
	 */
	lguest_pte_map_vcpu_data(vcpu, pte, LGUEST_HV_VCPU_DATA_PROT);
	lguest_pte_map_vcpu_guest_data(vcpu, pte, LGUEST_HV_VCPU_DATA_PROT);

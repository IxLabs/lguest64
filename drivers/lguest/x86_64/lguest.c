...
extern void lguest_swapgs(void);
...
struct lguest_vcpu_data *lguest_data_vcpu;
...
static void lguest_debug_write(struct console *con, const char *s, unsigned n)
{
	...
	hcall(LHCALL_PRINT, __pa(lguest_debug_buff), 0, 0);
}
...
void lguest_do_debug(u64 d, u64 data)
{
	d *= -1;
	if (lguest_paravirt)
		hcall(LHCALL_DEBUG_ME, d, data, 0);
}
...
{
	/* 
	 * We don't flush it when leaving lazy mode, because switching to
	 * host is a way too much expensive operation for us
	 */
	lazy_mode = mode;
}
...
static unsigned long save_fl(void)
{
	unsigned long flags;
	asm volatile (
		"movq " __stringify(LGUEST_VCPU_DATA_irq_enabled)"(%0), %0\n"
		: "=a" (flags)
		: "0" (lguest_data_vcpu));
	return flags;
}
...
static void restore_fl(unsigned long flags)
{
	 ...
	 */
	asm volatile (
		"movq %0,"__stringify(LGUEST_VCPU_DATA_irq_enabled)"(%1)\n"

		"testq %2,"
		__stringify(LGUEST_VCPU_DATA_flags)"(%1)\n"
		...
		:
		: "D" (flags),
		  "a" (lguest_data_vcpu), "i" (LGUEST_VCPU_IRQPEND_FL));
}

static void lguest_irq_disable(void)
{
	asm volatile (
		"movq $0," __stringify(LGUEST_VCPU_DATA_irq_enabled)"(%0)\n"
		:: "a" (lguest_data_vcpu));
}
...
{
	asm volatile (
		"movq $512," __stringify(LGUEST_VCPU_DATA_irq_enabled)"(%0)\n"
		"testq %1,"
		__stringify(LGUEST_VCPU_DATA_flags)"(%0)\n"
...
static __init void lguest_memory_setup(void)
{
	...
	add_memory_region(e820map->addr, e820map->size, e820map->type);
	return;
}
...
static void lguest_apic_write(unsigned long reg, unsigned int v)
{
	...
	switch (reg) {
	case APIC_ICR:
		hcall(LHCALL_APIC_WRITE, reg , v, cpu);
		break;
	case APIC_ICR2:
...
static void lguest_set_ldt(const void *addr, unsigned entries)
{
	/* FIXME: Implement. */
	if (entries)
		hcall(LHCALL_CRASH, __pa("set_ldt not supported"), 0, 0);
}
...
static void __init lguest_init_IRQ(void)
{
	...
	hcall(LHCALL_LOAD_IDT_ENTRY, CALL_FUNCTION_VECTOR, __pa((u64)&idt_table[CALL_FUNCTION_VECTOR]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+0, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+0]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+1, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+1]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+2, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+2]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+3, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+3]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+4, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+4]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+5, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+5]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+6, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+6]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, INVALIDATE_TLB_VECTOR_START+7, __pa((u64)&idt_table[INVALIDATE_TLB_VECTOR_START+7]), 0);
	hcall(LHCALL_LOAD_IDT_ENTRY, RESCHEDULE_VECTOR, __pa((u64)&idt_table[RESCHEDULE_VECTOR]), 0);
#endif
...
{
	/* FIXME: Allow this. */
	hcall(LHCALL_CRASH, __pa("write_ldt not supported"), 0, 0);
}
				   ...
				   unsigned type, unsigned size)
{
	...
	hcall(LHCALL_LOAD_GDT, __pa(ptr), GDT_ENTRIES, 0);
}

static void lguest_write_idt_entry(void *addr, struct gate_struct *s)
{
	native_write_idt_entry(addr, s);
}
...
int lguest_cpu_up(unsigned cpu)
{
	...
	hcall(LHCALL_NEW_VCPU, cpu, idle->thread.rsp, 
				(unsigned long)lguest_start_secondary);
...
void lguest_smp_send_stop(void)
{
	 ...
	 */
	hcall(LHCALL_STOP_VCPUS, smp_processor_id(), 0, 0);
}

void lguest_smp_send_reschedule(int cpu)
{
	hcall(LHCALL_REMOTE_CALL, 1, cpu, RESCHEDULE_VECTOR);
}
			     ...
			     int nonatomic, int wait)
{
	...
	hcall(LHCALL_REMOTE_CALL, 0, 0, CALL_FUNCTION_VECTOR);
				    ...
				    void *info, int nonatomic, int wait)
{
	...
	spin_lock(&call_lock);
	lguest_set_data(func, info, wait);
	hcall(LHCALL_REMOTE_CALL, 1, cpu, CALL_FUNCTION_VECTOR);
...
__init void lguest_init(void *boot)
{
	...
	paravirt_ops.irq_disable = lguest_irq_disable;
	paravirt_ops.irq_enable = lguest_irq_enable;
	...
	paravirt_ops.release_pgd = lguest_release_pgd;
	...
	smp_ops.smp_cpus_done = lguest_smp_cpus_done;
        smp_ops.smp_send_stop = lguest_smp_send_stop;
	...
	*(unsigned short *)(__va(0x40E)) = 0;
	...
	lguest_data_vcpu = (struct lguest_vcpu_data*)lguest_data.vcpu_shared_data;
	...
	paravirt_ops.swapgs = lguest_swapgs;

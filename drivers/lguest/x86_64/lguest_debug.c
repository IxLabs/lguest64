...
void lguest_dump_vcpu_regs(struct lguest_vcpu *vcpu)
{
	...
	unsigned long stack;
	unsigned long rsp = guest_pa(vcpu->guest, regs->rsp);
	int i;
	...
	printk("RSP: %04llx:%016llx  EFLAGS: %08llx irqs %s\n", regs->ss, regs->rsp,
	       regs->rflags,
	       vcpu->vcpu_data->irq_enabled?"enabled":"disabled");
	...
	printk("FS %04llx: Base: %016lx  Desc: %08lx\n",
	       regs->fs,(vcpu->vcpu_data->guest_fs_a |
			(vcpu->vcpu_data->guest_fs_d << 32)), 
			(vcpu->vcpu_data->guest_fs_desc_a |
			(vcpu->vcpu_data->guest_fs_desc_d << 32)));

	data_cr2 = vcpu->vcpu_data->cr2;

	SAVE_CR2(cr2);
	printk(" CR2: %lx  lguest_vcpu_data->cr2: %lx\n", cr2, data_cr2);
	
	gs = (u32)vcpu->vcpu_data->guest_gs_a | (vcpu->vcpu_data->guest_gs_d << 32);
	shadow_gs = (u32)vcpu->vcpu_data->guest_gs_shadow_a |
...
void lguest_dump_trace(struct lguest_vcpu *vcpu, struct lguest_regs *regs)
{
	...
	printk("Stack Trace:\n");
	if (stack < vcpu->guest->page_offset) {
		printk("  <USER STACK>\n");
		goto out;
	}
	...
	for (; stack < stack_end; stack += sizeof(stack)) {
		addr = lgread_u64(lg, guest_pa(vcpu->guest, stack));
...
#if 0
void lguest_check_hv_pages(struct lguest_vcpu *vcpu)
{
	...
	pgd = __va(vcpu->regs.cr3 & PAGE_MASK);
	BUG_ON(!pgd);
...
static void lguest_dump_page_tables(struct lguest_vcpu *vcpu)
{
	printk("Host page tables:\n");
	lguest_print_page_tables(vcpu->pgd->hcr3);

	printk("\n\nGuest page tables:\n");
	lguest_print_guest_page_tables(vcpu, vcpu->pgd->gcr3);
}

static int lguest_test_pte(struct lguest_vcpu *vcpu, int pgd_idx, int pud_idx,
			   int pmd_idx, u64 *pte, int check)
{
	...
	for (p=0; p < PTRS_PER_PTE; p++) {
		if (pte[p] & _PAGE_PRESENT) {
			...
			gpaddr = lguest_find_guest_paddr(vcpu, vaddr);
			if (gpaddr == (u64)-1) {
				if (!check)
					return 1;
				/* 
				...
				printk("WARNING! cr3: %llx pgd: %p page %llx is mapped\n"
				       ...
				       vaddr);
				printk("rip=");
				lguest_print_address(vcpu, vcpu->vcpu_data->regs.rip);
				printk(" (%lx) pte[%x] = %llx\n",
				       __pa(pte), p, pte[p]);
				check = 0;
				continue;
...
static int lguest_test_pmd(struct lguest_vcpu *vcpu, int pgd_idx, int pud_idx,
			   u64 *pmd, int check)
{
	int m;
	int ret;
	...
	for (m=0; m < PTRS_PER_PMD; m++) {
		if (pmd[m] & _PAGE_PRESENT) {
			ret = lguest_test_pte(vcpu, pgd_idx, pud_idx,
...
static int lguest_test_pud(struct lguest_vcpu *vcpu, int pgd_idx, u64 *pud, int check)
{
	int u;
	int ret;
	...
	for (u=0; u < PTRS_PER_PUD; u++) {
		if (pud[u] & _PAGE_PRESENT) {
			ret = lguest_test_pmd(vcpu, pgd_idx,
...
static int lguest_test_pages(struct lguest_vcpu *vcpu, u64 *pgd, int check)
{
	int g;
	int ret;
	int ignore = 0;
	...
	for (g=0; g < PTRS_PER_PGD; g++) {
		if (pgd[g] & _PAGE_PRESENT) {
			ret = lguest_test_pud(vcpu, g, __va(pgd[g] & PTE_MASK), check);
			if (!ret)
				continue;
			if (ret == 1) {
				check = 0;
			...
			if (ret < 0) {
				printk(" (%lx) pgd[%x] = %llx\n",
				       __pa(pgd), g, pgd[g]);
				if (!ignore) {
					lguest_dump_page_tables(vcpu);
					kill_guest_dump(vcpu, "bad page mapping\n");
				}
				return -1;
			}
		}
...
{
	lguest_test_pages(vcpu, vcpu->pgd->hcr3, check);
}
